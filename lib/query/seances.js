...
const dict = require("../../config/main.js").dict;
...
async function getSeancesFromCats(db, idCats, idProg) {
  let seancesFilms = await execQuery.single(db, queries.seancesFilmsFromCats, [
    idCats,
    idProg,
  ]);
  let seancesConfs = await execQuery.single(db, queries.seancesConfsFromCats, [
    idCats,
    idProg,
  ]);
  ...
  //         idSalle: dict.salles[d[0].idSalle]
  //       })
  //       .value();
  //   })
  //   .value();
    ...
    .mapValues((d) => {
      return _({})
        .assign(
          ...
          {
            items: _(d)
              .map((e) => _(e).pick(["idFilm", "ordre"]).value())
              .sortBy("ordre")
              .groupBy("idFilm")
              .mapValues((e) => e[0])
              .value(),
...
async function getSeancesMentionsFromCats(db, idCats, idProg) {
  let seancesMentions = await execQuery.single(
    db,
    queries.seancesMentionsFromCats,
    [idCats, idProg]
  );
      ...
      _(d)
        ...
        .mapValues((c) =>
          _(c)
            ...
            .map(
              (b) =>
                format.formatName(b.prenom, b.particule, b.nom) +
                (_.kebabCase(b.note) === "sous-reserve"
                  ? " (sous réserve)"
                  : "")
            )
            .value()
        ...
        .map((v, k) => (dict.mentionSeance[k] || "") + v)
        .value()
    )
    .mapValues((c) => c.join(". "))
    .mapValues((c) => ({
...
async function getSeancesCopiesFromCats(db, idCats, idProg) {
  let seancesCopies = await execQuery.single(
    db,
    queries.seancesCopiesFromCats,
    [idCats, idProg]
  );
      ...
      _(d)
        ...
        .mapValues((e) =>
          _(e)
            .assign({
              sousTitres:
                e.stDemande === 1 && !e.intertitres ? 23 : e.sousTitres, // Sous-titres demandés équivaut à STF
              intertitres:
                e.stDemande === 1 && e.intertitres ? 25 : e.intertitres, // Avec intertitres : sous-titres demandés équivaut à INT. FR.
            })
            .value()
        )
        .mapValues((e) =>
          _(e)
            .assign({
              version:
                (dict.version[e.version] || "") +
                  (dict.sousTitres[e.sousTitres] || "") ||
                dict.intertitres[e.intertitres] ||
                "",
              format: dict.format[e.format] || "",
            })
  ...
  seancesMerged = _(seancesMerged)
    .thru((data) => {
      ...
      return _(data).reduce((acc, i) => {
        if (!i.idSeanceAssoc || i.typeAssoc === 117) {
          // Essai : ignorer aussi les associations de type séances consécutives
  ...
  seancesMerged = _(seancesMerged)
    .map((d) => {
      if (d.mention || d.typeSeance) {
        d = _(d)
          .assign({
            mention: format.joinLast(". ", null, [
              dict.typeSeance[d.typeSeance],
              d.mention,
            ]),
          })
          .value();
