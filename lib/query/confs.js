...
module.exports = async function(db, cycleConfig) {
  ...
  confsMerge = _(confsMerge)
    .map(d => {
      // Pour les conférences (idTypeConference 55), on crée un sousTitre de la forme "Conférence de ..." (journal 05/07/2019)
      if (d.idTypeConference === 55) {
        ...
        if (intervenants.length === 0)
          console.log(
            `AVERTISSEMENT : L'évenement (conférence) ${d.idEvenement} n'a pas d'intervenants.`
          );

        intervenants = format.joinLast(", ", " et ", intervenants);
    ...
    .map(d => {
      ...
      if (intervenants.length > 0) {
        return _({})
          .assign(d, {
            mentionAnimation: `Rencontre animée par ${format.joinLast(
              ", ",
              " et ",
              intervenants
            )}`
