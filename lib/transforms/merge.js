...
function mergeFilmsSeances(cycleConfig, films, seances, confs, texts) {
  ...
  if (diffFilmsSeances.length === 0 && diffSeancesFilms.length === 0) {
    ...
    if (diffSeancesFilms.length > 0) {
      ...
      console.log(
        `AVERTISSEMENT : Les films suivants ont des séances mais pas de données les concernant : ${diffSeancesFilms.join(
          ", "
        )}`
      );
    }
  ...
  cycle = _(cycleConfig.sousCycles)
    .map((d) => {
      return {
        ...
        tri: d.tri,
        items: _(d.cats)
          .map((e) => _(cycle).groupBy("idCategorie").value()[e])
      ...
      return _(d)
        .assign({
          textes: _(d.items)
            ...
            .uniq()
            .flatten()
            .map((e) => (_.isUndefined(texts) ? [] : texts.cats[e]))
            // .map(e => texts.cats[e])
            .filter()
            .flatten()
            .value(),
