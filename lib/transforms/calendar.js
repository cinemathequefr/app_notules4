...
const pages = require("../query/pages.js");
  ...
  <% _.forEach(dayProg, seance => { %>
    ...
    <% _.forEach(seance.items, (item, i) => { %>
      ...
      <% if (item.realisateurs || item.duree || item.version) { %>
        ...
        <% } %>
      <% } %>
      [SPACE]<cstyle:FOLIO>[p.<0x2009><%= item.page ? item.page : "xx" %>]<cstyle:>[CRLF]
    <% }) %>
    <% if (seance.mention) { %>
...
`
  ...
  try {
    ...
    // console.log(evenementsPages);
    console.log("Les informations de foliotage ont été obtenues.");
    // On rattache à chaque séance son n° de page.

    // console.log(JSON.stringify(evenementsPages, null, 2));
    // console.log(JSON.stringify(data["2022-07-14"], null, 2));

    data = _(data)
      .mapValues((day) =>
        _(day)
          .map((seance) => {
            let page = _(evenementsPages).find(
              (e) => e.idevenement === seance.idEvenement
            );
            page = page ? page.numeroPage : null;
    ...
    //               .assign(seance, {
    //                 items: _(seance.items)
    //                   .map((item) => _({}).assign(item, page).value())
    //                   .value(),
    //               })
    //               .value();
    //           })
