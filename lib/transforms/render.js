...
function render(cycle) {
  cycle = _(cycle)
    .map((d) =>
      _(d)
        .pick(["titreSousCycle", "tri", "textes"])
        .assign({
                  ...
                  .map((d) =>
                    _(d[0])
                      ...
                      .thru((d) => {
                        if (
                          _(d.seance).every(
                            (e) => e.precedeSuivi === d.seance[0].precedeSuivi
                          ) &&
                          !_.isUndefined(d.seance[0].precedeSuivi)
                        ) {
                          return _({})
                            .assign(d, {
                              precedeSuivi: d.seance[0].precedeSuivi,
                              seance: _(d.seance)
                                .map((e) => _(e).omit(["precedeSuivi"]).value())
                                .value(),
                            })
                            .value();
                        } else {
                ...
                return _(items)
                  ...
                  .thru((d) => {
                    if (tri === 2) {
                      return _(d)
                        .orderBy([
                          (e) => _.kebabCase(e.films[0].titre),
                          (e) => parseInt(/(\d+)/.exec(e.titreEvenement)),
                        ])
                        .value();
                    }
                    if (tri === 3) {
