...
const stripInvalidFilenameChars =
  require("./format.js").stripInvalidFilenameChars;
 ...
 */
function extractArgsValue(args, noKeyDefaultName) {
  let o = [];
  let m = null;
...
function cycleConfig(cyclesConfig, idCycle) {
  try {
    let c = _(
      ...
      .tap((d) => {
        if (_.isUndefined(d))
          throw `ERREUR : Les données de configuration pour le cycle ${idCycle} n'ont pas été trouvées.`;
      })
      .assign({
    ...
    if (_.uniq(cats).length !== cats.length) {
      throw `ERREUR : Les données de configuration du cycle ${idCycle} ont des catégories en doublon.`;
    }
    ...
    if (extCats.length > 0) {
      console.log(
        `AVERTISSEMENT : les données du configuration du cycle ${idCycle} listent des catégories présentes dans d'autres cycles du programme : ${extCats.join(
          ", "
        )}.`
      );
    }
    ...
    console.error(e);
    // Les erreurs attrapées ici sont fatales : on interrompt le processus.
    process.exit(1);
  }
}
...
const getFullCode = {
  ...
  cycle: function (progConfig, idCycle) {
    try {
      let titreCycle = _(progConfig.cycles).find(
        (d) => d.idCycleProg === idCycle
      ).titreCycle;
      return [
