...
/**
 * pagesFromCats
 * Liste l'association idEvenement/n° de page pour les catégories données (= a priori, celles d'un programme).
 * Utilisé par calendar.js pour l'export du calendrier pour InDesign.
 * @param {} params
 */
function pagesFromCats(params) {
  let idCats = JSON.stringify(params[0]);
  idCats = idCats.substring(1, idCats.length - 1);
  return `
    SELECT
      prog_evenement.idevenement,
      prog_evenement.numero_page
    FROM
      prog_evenement
      INNER JOIN prog_affect_evenement_categorie ON prog_affect_evenement_categorie.idevenement = prog_evenement.idevenement
      WHERE prog_evenement.numero_page IS NOT NULL and prog_affect_evenement_categorie.idcategorie IN (${idCats})
  `;
}
