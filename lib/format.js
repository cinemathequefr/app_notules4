...
function normalizeTitle(titleVo, artVo, titleFr, artFr, titleFrMod, artFrMod) {
  ...
  // TODO: retirer une éventuelle mention [TV].
  output = _(output)
 ...
 * La liste des pays se trouve dans le tableau `config.main.countries`
 * @param codes {string}
 * @output {array}
 */
function expandCountries(codes) {
  return _(codes.split(";"))
    .map(
      (code) => _.find(config.main.countries, (f) => f[0] === code)[1] || code
    )
    .value();
}
...
function precedeSuivi(avant, apres) {
  avant = joinLast(
    ...
    _(avant).map(function (d) {
      return (
        "_" +
        artTitre(d.art, d.titre) +
        "_" +
        beforeAfterStr(" " + de(d.realisateurs), "", d.realisateurs)
      );
    })
  );
  apres = joinLast(
