...
(async () => {
  ...
  try {
    let md = await helpers.readFile(
      `${basePath}/${progDirectoryName}`,
      `${cycleFullCode[0]} ${cycleFullCode[1]}`,
      `${cycleFullCode[0]}_TEXTE ${cycleFullCode[1]}.md`
    );
...
${tt}`;
    ...
    await helpers.writeFileInFolder(
      `${basePath}/${progDirectoryName}``${cycleFullCode[0]} ${cycleFullCode[1]}`,
      `${cycleFullCode[0]}_TEXTE ${cycleFullCode[1]}.txt`,
      iconv.encode(tt, "win1252")
    );
